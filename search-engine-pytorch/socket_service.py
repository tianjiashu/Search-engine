...
img_emb = torch.tensor(np.asarray(total_emb),device=device)
img_urls = total_urls
def search(query, k=8):
    # 首先，我们对查询进行编码(它可以是图像或文本字符串)
    query_emb = text_model.encode([query], convert_to_tensor=True, show_progress_bar=False,device=device)

    '''
    然后，我们使用util.Semantic_search函数，它计算查询嵌入和所有图像嵌入之间的余弦相似度。
    然后它返回top_k排名最高的图像，我们输出这些图像
    '''
...
def search_img(img_file,k=8):
    input_img_emb = model.encode(Image.open(img_file) , convert_to_tensor=True, show_progress_bar=False,device=device)
...
while True:
    ...
    while True:
        request_data = tcpClientSock.recv(BUFSIZ)#阻塞
        if not request_data:
            break
